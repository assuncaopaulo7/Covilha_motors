...
public class AdminController {
    ...
    private final Path          uploadDir;           // onde gravamos imagens
    ...
    public AdminController(CarRepository carRepo,
        ...
        Files.createDirectories(uploadDir);           // cria se não existir
    }
    ...
    public String addCar(HttpSession ses,
        ...
        String savedName = saveImage(image);   // pode devolver null

        Car car = new Car();
        car.setBrand(brand); car.setModel(modelCar);
        car.setCategory(category); car.setPrice(price); car.setStock(stock);
        car.setImagePath(savedName);
        carRepo.save(car);

        logService.logCar(admin.getEmail(), "CRIOU", car, stock);
        return "redirect:/admin";
    }

    /* ---------- SUBSTITUIR / ADICIONAR IMAGEM ---------- */
    @PostMapping(path="/cars/{id}/image",
            consumes=MediaType.MULTIPART_FORM_DATA_VALUE)
    ...
    public String updateCar(HttpSession ses,
        ...
        carRepo.findById(id).ifPresent(car -> {
            ...
            if (diff > 0)  logService.logCar(admin.getEmail(), "INSERIU",  car, diff);
            if (diff < 0)  logService.logCar(admin.getEmail(), "RETIROU", car, -diff);
        });
        return "redirect:/admin";
    }
    ...
    public String deleteCar(HttpSession ses, @PathVariable Long id) {
        ...
        carRepo.findById(id).ifPresent(car -> {
            ...
            /* soft-delete: marca como removido, mas mantém o registo na DB */
            car.setDeleted(true);
            carRepo.save(car);
        });
        return "redirect:/admin";
